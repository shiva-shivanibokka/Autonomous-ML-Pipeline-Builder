...
SYSTEM_PROMPT = build_system_prompt(
    ...
    context=(
        "You will write a complete Python preprocessing script. "
        "The script must:\n"
        "1. Load the CSV from the path variable `INPUT_CSV_PATH`\n"
        "2. Apply all recommended preprocessing steps\n"
        "3. Save the transformed DataFrame to `OUTPUT_CSV_PATH`\n"
        "4. Print a summary: number of rows, columns, and any new features created\n\n"
        "Rules:\n"
        "- Use only: pandas, numpy, scikit-learn (already installed)\n"
        "- NEVER import the target column into scaling/encoding (prevent leakage)\n"
        "- Use pd.get_dummies for low-cardinality categoricals (<= 10 unique values)\n"
        "- Use label encoding or target encoding for high-cardinality categoricals\n"
        "- Use SimpleImputer(strategy='median') for numeric, 'most_frequent' for categorical\n"
        "- Use StandardScaler for numeric features (not the target column)\n"
        "- For datetime columns: extract year, month, day, dayofweek, is_weekend\n"
        "- For class imbalance: do NOT apply SMOTE here (handle in training)\n"
